...
func add_from_stat(stats: Stats):
	var status_effect = status_effect_scene.instantiate()
	status_effect.stats = stats.duplicate()
...
func on_effect_proc(status_effect: Stats):
	effect_proc.emit(status_effect)
	pass
	
func on_effect_duration(status_effect: Stats):
